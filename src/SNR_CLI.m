...
function SNR_CLI(wavDirectory, csvDirectory, matchesDirectory)
        ...
        writelines("Contour,Clip,SNR_Noise_Across_Time, SNR_Noise_Across_Frequency", snr_file, "WriteMode","overwrite");
        ...
        for i = 1:height(match_table)
            ...
            if (not (contains(row.("CSV"), "No matches")))
                ...
                [SNR_NoiseAcrossFreq, SNR_NoiseAcrossTime, ~, ~, ~, ~] = computeSNR(csvPath, wavPath);
                ...
                snrVals = strcat( strcat( string(SNR_NoiseAcrossTime), "," ), string(SNR_NoiseAcrossFreq) );
